...
SERVICE_ACCOUNT_FILE = (
    "keygoogle/email-parsing-406909-231994f0fedc.json"
)
...
def parse_date(date_str):
    if not date_str:
        return None
    try:
        # Define a custom timezone info for PST (UTC-8)
        tzinfos = {"PST": timezone(timedelta(hours=-8))}
        # Parse the date with the provided tzinfos
        return parser.parse(date_str, fuzzy=True, tzinfos=tzinfos).date().isoformat()
